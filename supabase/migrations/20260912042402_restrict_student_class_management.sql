...
alter policy tracer_classes_insert on public.classes with check (
  created_by = (select auth.uid()) and (select private.can_manage_classrooms())
);
alter policy tracer_classes_update on public.classes
using ((select private.can_manage_classrooms()) and (select private.can_manage_class(id)))
with check ((select private.can_manage_classrooms()) and (select private.can_manage_class(id)));
alter policy tracer_classes_delete on public.classes using (
  created_by = (select auth.uid()) and (select private.can_manage_classrooms())
);
alter policy tracer_memberships_insert on public.class_memberships with check (
  (select private.can_manage_classrooms()) and (
    (select private.is_class_teacher(class_id))
    or (user_id = (select auth.uid()) and role = 'teacher'
      and (select private.can_manage_class(class_id)))
  )
);
alter policy tracer_memberships_update on public.class_memberships
using ((select private.can_manage_classrooms()) and (select private.is_class_teacher(class_id)))
with check ((select private.can_manage_classrooms()) and (select private.is_class_teacher(class_id)));
alter policy tracer_memberships_delete on public.class_memberships
using ((select private.can_manage_classrooms()) and (select private.is_class_teacher(class_id)));
alter policy tracer_assignments_insert on public.assignments with check (
  (select private.can_manage_classrooms()) and created_by = (select auth.uid())
...
using ((select private.can_manage_classrooms()) and (select private.can_manage_assignment(id)))
with check ((select private.can_manage_classrooms()) and (select private.is_class_teacher(class_id))
  and (select private.can_read_set_version(set_version_id)));
