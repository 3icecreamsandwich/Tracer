...
begin
  ...
  insert into public.class_memberships (class_id, user_id, role, status, joined_at)
  values (matched_class.id, current_user_id, 'student', 'active', pg_catalog.now())
  ...
  return query
  select matched_class.id, matched_class.name, matched_class.subject,
    matched_class.section, matched_class.school_year, matched_class.timezone,
    matched_class.created_at;
end;
$function$;

revoke all on function private.join_tracer_class(text) from public, anon;
grant execute on function private.join_tracer_class(text) to authenticated;
...
set search_path = ''
as $function$
  select * from private.join_tracer_class(requested_code);
$function$;
