...
create table if not exists public.private_sets (
...
drop policy if exists "Users read their own private sets" on public.private_sets;
create policy "Users read their own private sets" on public.private_sets
  for select to authenticated using ((select auth.uid()) = owner_id);
drop policy if exists "Users create their own private sets" on public.private_sets;
create policy "Users create their own private sets" on public.private_sets
  for insert to authenticated with check ((select auth.uid()) = owner_id);
drop policy if exists "Users update their own private sets" on public.private_sets;
create policy "Users update their own private sets" on public.private_sets
  for update to authenticated using ((select auth.uid()) = owner_id)
  with check ((select auth.uid()) = owner_id);
drop policy if exists "Users delete their own private sets" on public.private_sets;
