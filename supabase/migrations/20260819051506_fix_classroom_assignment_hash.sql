...
create index if not exists set_versions_created_by_idx on public.set_versions (created_by);
create index if not exists set_assets_uploaded_by_idx on public.set_assets (uploaded_by);
create index if not exists sets_current_version_idx on public.sets (current_version_id);

insert into storage.buckets (id, name, public, file_size_limit, allowed_mime_types)
values ('set-assets', 'set-assets', false, 10485760, array['image/jpeg', 'image/png', 'image/gif', 'image/webp', 'image/avif']::text[])
...
drop trigger if exists assign_existing_class_materials_after_membership on public.class_memberships;
...
begin
  ...
  if cloud_set_id is null then
    insert into public.sets (
      owner_id, local_source_id, title, description, visibility, source_type
    )
    values (
      current_user_id,
  ...
  if cloud_version_id is null then
    ...
    insert into public.set_versions (
      set_id, version_number, content, content_hash, created_by
    )
    values (
      cloud_set_id, next_version, requested_content, computed_content_hash, current_user_id
    )
    returning id into cloud_version_id;
  end if;
  ...
  insert into public.set_assets (
    bucket_id, object_path, set_version_id, uploaded_by
  )
  ...
  returning id, created_at into cloud_assignment_id, cloud_assigned_at;
