<template>
    ...
    >
        <template v-if="allowCopying">
            <label for="share-terms" class="sr-only">{{
                t("public.share")
            }}</label>
            <textarea
                id="share-terms"
        ...
        <div class="mt-5 flex flex-wrap gap-2">
            ...
            <AppButton
                ...
                :busy="publishing"
                @click="publish"
                ><AppIcon name="publish" /><span class="text-white font-bold dark:text-black">{{
        ...
        >
            ...
            <label
                class="flex cursor-pointer items-center justify-between gap-4 text-sm font-medium"
...
async function publish() {
    ...
    message.value = "";
    try {
        publishedId.value = await publishSet(
            props.set,
            tags.value,
            copying.value,
        );
        ...
        error.value =
            e instanceof Error ? e.message : t("public.publishFailed");
    } finally {
        publishing.value = false;
    }
}
