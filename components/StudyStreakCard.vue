<template>
  ...
  <section v-else class="overflow-hidden rounded-2xl border border-orange-200/90 bg-white shadow-sm dark:border-orange-400/20 dark:bg-slate-900" aria-label="Study streak">
    ...
    <div class="px-4 py-4">
      <div class="hidden grid-cols-7 gap-1.5 sm:grid">
        <button v-for="day in displayedWeek" :key="day.key" type="button" class="group flex min-w-0 flex-col items-center rounded-xl px-1 py-2 transition focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-orange-500 focus-visible:ring-offset-2 dark:focus-visible:ring-offset-slate-900" :class="selected === day.key ? 'bg-orange-50 dark:bg-orange-500/10' : 'hover:bg-slate-50 dark:hover:bg-slate-800'" :aria-pressed="selected === day.key" @click="selected = day.key">
          ...
          <span class="mt-1.5 h-1 w-4 rounded-full" :class="day.complete ? 'bg-orange-400' : day.seconds > 0 ? 'bg-amber-300 dark:bg-amber-500' : 'bg-transparent'" />
        </button>
      </div>
