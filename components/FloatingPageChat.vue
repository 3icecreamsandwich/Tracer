<template>
    <div v-if="floatingChatReady && floatingChatEnabled" data-page-chat-root>
        ...
        >
            ...
            <div
                ref="messagesEl"
                class="m-3 min-h-0 flex-1 space-y-3 overflow-y-auto rounded-md border border-amber-200 bg-amber-50/20 p-3 text-sm text-slate-900 shadow-sm dark:border-amber-900/60 dark:bg-amber-950/10 dark:text-slate-50"
                aria-live="polite"
            >
                <div
            ...
            <div
                v-if="error"
                class="border-t border-red-100 bg-red-50 px-4 py-2.5 text-xs text-red-700 dark:border-red-950 dark:bg-red-950/40 dark:text-red-200"
            >
                {{ error }}
                <NuxtLink
                    v-if="missingModel"
...
function onDocumentPointerDown(event: PointerEvent) {
    ...
    if (panelEl.value?.contains(target) || triggerEl.value?.contains(target))
        return;
    closePanel();
}
    ...
    const controls = Array.from(
        ...
        .map((control) => {
            ...
            return control.value.trim()
                ? `${label}: ${control.value.trim()}`
                : "";
        })
        .filter(Boolean);

    if (controls.length)
        sections.push(`Current form values:\n${controls.join("\n")}`);
