...
test('set page learn: generates questions without an explicit seed', async ({ page }) => {
  await page.goto('/set/demo?mode=learn')

  await expect(page.getByRole('region', { name: 'Practice' })).toBeVisible()
  await expect(page.getByText('Failed to generate questions.')).toHaveCount(0)
