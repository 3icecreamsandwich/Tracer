...
export default defineConfig({
  ...
  use: {
    baseURL: `http://127.0.0.1:3101${basePath}`,
    browserName: 'chromium',
    launchOptions: process.env.TRACER_TEST_BROWSER ? { executablePath: process.env.TRACER_TEST_BROWSER } : undefined,
    screenshot: 'only-on-failure',
  },
  webServer: {
    command: production ? 'node .output/server/index.mjs' : 'bun --bun node_modules/nuxt/bin/nuxt.mjs dev --host 127.0.0.1 --port 3101',
